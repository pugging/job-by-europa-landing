...
export default function Home() {
  return (
    <div className="min-h-screen">
          ...
          <nav className="nav-island">
            ...
            <div className="hidden md:flex items-center gap-6">
              {["Features", "How it Works", "Results"].map((l) => (
                <a key={l} href={`#${l.toLowerCase().replace(/ /g, "-")}`} className="text-sm text-[var(--color-text-muted)] hover:text-[var(--color-foreground)] transition-colors">
                  {l}
                </a>
              ))}
            </div>
      ...
      {/* ===== HERO ===== */}
      <section className="section pt-32 md:pt-44 pb-8">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="heading-display reveal-up">Capture jobs</h1>
            <h2 className="heading-display heading-muted reveal-up-d1">Automate the rest</h2>
            ...
            <div className="flex items-center justify-center gap-3 mt-10 reveal-up-d3">
              <PlanarButton href="#download">Download Extension <Arrow /></PlanarButton>
            ...
            <div className="marquee-track">
              {[...Array(2)].map((_, i) => (
                <div key={i} className="flex items-center gap-16">
                  {["pracuj.pl", "LinkedIn", "NoFluffJobs", "Indeed", "JustJoin.it", "Bulldogjob", "RocketJobs"].map((n) => (
                    <span key={`${i}-${n}`} className="text-lg md:text-xl font-semibold tracking-tight whitespace-nowrap opacity-25 hover:opacity-50 transition-opacity">{n}</span>
                  ))}
                </div>
              ))}
            </div>
      ...
      <section id="results" className="section">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-lead mt-4 max-w-lg mx-auto">Real impact on your job search workflow.</p>
          </div>
      ...
      <section id="download" className="section">
        <div className="max-w-4xl mx-auto px-4 md:px-8">
          <div className="cta-section">
            <p className="text-xs uppercase tracking-widest opacity-40 font-medium mb-6">Free & open source</p>
            <h2 className="heading-display text-3xl md:text-5xl">
              Ready to snipe your<br />dream job?
            </h2>
            <p className="text-lead mt-4 opacity-60 max-w-lg mx-auto" style={{ color: "white" }}>
              Download the extension, connect your n8n, and start automating your job search. Built by a student, for students.
            </p>
            <div className="flex items-center justify-center gap-3 mt-10">
              <PlanarButton href="https://github.com/pugging/hr-sniper-landing" target="_blank" rel="noopener noreferrer" inverted>
...
const features = [
  ...
  {
    icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>,
  ...
  {
    icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>,
    title: "Privacy First",
  ...
  {
    icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>,
...
const steps = [
  { num: "01", title: "Install the extension", desc: "Download from GitHub and add to Chrome. Set your n8n webhook URL once — done forever." },
  { num: "02", title: "Browse job listings", desc: "Navigate to any job posting on Pracuj.pl, LinkedIn, NoFluffJobs, Indeed — or any website." },
  { num: "03", title: "Click & automate", desc: "Hit the analyze button. Job data flies to n8n where your AI workflow generates insights instantly." },
];
      ...
      <div className="bg-white rounded-2xl shadow-[0_20px_60px_-10px_rgba(0,0,0,0.1)] border border-[var(--color-border)] overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="text-[11px] text-[var(--color-text-muted)] uppercase tracking-wider font-medium mb-1">Job listing detected</div>
              <div className="font-semibold tracking-tight">Frontend Developer</div>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {["React", "TypeScript", "Next.js", "8–12k PLN"].map((t) => (
              <span key={t} className="tile text-xs font-medium">{t}</span>
            ))}
          </div>

          <button className="w-full bg-[var(--color-foreground)] text-white rounded-xl py-3 text-sm font-medium flex items-center justify-center gap-2 transition-opacity hover:opacity-90">
